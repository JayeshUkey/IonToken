...
contract RewardToken {
    ...
    function updateTime(address account) private validAccount(account) {
        lastUpdateTime[account] = now();
    }

    function updateUsersRewardCycle(address account) private validAccount(account) {
    ...
    // RewardRate nedds to be referred for rewardRate Balance
    ...
    function setMinimumStakingPeriod(uint256 minimumStakingPeriodInSeconds) external onlyOwner timestampNotSet {
        require(timePeriod > 899, "To account for potential 900 second error of block.timestamp you must set timestamp to minimum 900 seconds");
        initialTimestamp = now();
        timePeriod = initialTimestamp.add(timePeriod);
    }
